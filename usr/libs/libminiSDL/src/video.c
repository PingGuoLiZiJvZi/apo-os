...
void SDL_FreeSurface(SDL_Surface *s)
{
  ...
  if (s->pixels && !(s->flags & SDL_PREALLOC))
    free(s->pixels);
  free(s);
}
...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect)
{
  ...
  for (int y = 0; y < sh; y++)
  {
    if (sy + y < 0 || sy + y >= src->h || dy + y < 0 || dy + y >= dst->h)
      continue;
  ...
  if (s->format->BitsPerPixel == 32)
  {
    ...
    else
    {
      ...
      for (int row = 0; row < h; row++)
      {
        ...
        memcpy(tmp + row * w, src, (size_t)w * sizeof(uint32_t));
      }
...
{
  (void)flags;
  if (!s || !s->format || !s->format->palette || !colors)
    return;
  for (int i = 0; i < ncolors; i++)
  {
    int idx = firstcolor + i;
    if (idx < 0 || idx >= s->format->palette->ncolors)
      continue;
