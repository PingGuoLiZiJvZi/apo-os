...
static const char *resolve_rom_path(int argc, char *argv[], char *buf, size_t bufsz) {
	const char *arg = 0;
	...
	size_t n = strlen(arg);
	if (n >= 4 && strcmp(arg + n - 4, ".nes") == 0) {
		snprintf(buf, bufsz, "/share/games/nes/%s", arg);
	} else {
		snprintf(buf, bufsz, "/share/games/nes/%s.nes", arg);
	}
	return buf;
}
...
int main(int argc, char *argv[])
{
  ioe_init();
	...
	char rompath[256] = {0};
	const char *romname = resolve_rom_path(argc, argv, rompath, sizeof(rompath));
