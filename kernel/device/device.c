...
static int is_desktop_proc(void) {
...
static int read_events_device(void *buf, size_t n) {
    ...
    int is_desktop = is_desktop_proc();
...
static int real_fb_write(uint32_t *off, const void *buf, size_t n) {
    int w = 0;
    int h = 0;
...
int device_fs_read(const char *name, uint32_t *off, void *buf, size_t n) {
    ...
    if (strcmp(name, "fbsync") == 0) {
        return virtual_gpu_fbsync_read(buf, n);
    }
...
int device_fs_write(const char *name, uint32_t *off, const void *buf, size_t n) {
    ...
    if (strcmp(name, "input") == 0) {
        if (!is_desktop_proc()) return -1;
        if (n >= sizeof(int)) {
            int focus_pid = *(const int *)buf;
            input_focus_set(focus_pid);
        }
    ...
    if (strcmp(name, "fbsync") == 0) {
        if (is_desktop_proc()) {
            ...
            if (virtual_gpu_fbsync_write(current_proc ? current_proc->display : 0, buf, n) < 0) {
                return -1;
            }
        }
        if (off) *off += (uint32_t)n;
    ...
    if (strcmp(name, "fb") == 0) {
        if (is_desktop_proc()) {
            return real_fb_write(off, buf, n);
        }
        return virtual_gpu_fb_write(current_proc ? current_proc->display : 0, off, buf, n);
    }
...
uint64_t device_mmap_size(const char *name) {
    ...
    if (strcmp(name, "fb") == 0) {
        ...
        if (is_desktop_proc()) {
            return fb_sz * (1 + VIRTUAL_GPU_MAX_DISPLAYS);
        }
        return fb_sz;
    }
    return 0;
...
int device_mmap_page(const char *name, uint64_t offset, uint64_t *pa) {
    ...
    if (strcmp(name, "fb") == 0) {
        ...
        if (is_desktop_proc()) {
            ...
            return virtual_gpu_existing_fb_page(PCBs[child_idx].display, page_off, pa);
        }

        return virtual_gpu_fb_page(current_proc ? current_proc->display : 0, offset, pa);
    }
    return -1;
}
